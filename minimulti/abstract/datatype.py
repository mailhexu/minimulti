...
class Rijv(defaultdict):
    def __init__(self, default_factory=None, shape=[], sparse=False, dtype=float):
    ...
    def assure_asr(self):
        smat = sum(self.values())
        mat0 = copy.deepcopy(self[(0, 0, 0)])
        d=np.sum(smat, axis=0)
        mat0 -= np.diag(d)
        self[0, 0, 0] = mat0
    ...
    def make_supercell(self, scmaker, sparse=False, tol=1e-4):
        ...
        nbasis = self.shape[0]
        ret = Rijv(shape=(sc_nbasis, sc_nbasis),
                   sparse=sparse,
                   dtype=self.dtype)
        print(len(self))
        print(sc_nbasis)
