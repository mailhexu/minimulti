...
class Hubbard_matrix_term(object):
    ...
    def V_and_E(self, rho):
        ...
        for g, UJ_ind in self._corr_group.items():
            ...
            rho_site = rho.block(ids)
            V_int_site, E_int_site = self._Int_site(rho_site,
                                                    U4=U4ind,
                                                    U2=U2,
                                                    U2prime=U2prime)
            Vee[ind] += V_int_site
            ...
            V_DC_site, E_DC_site = self._DC_site(U, J, U2, U2prime, rho_site)
            Vee[ind] += V_DC_site
            E_DC += E_DC_site

            E_eig += -np.sum(Vee[ind] * rho_site)
        E = E_eig + E_int + E_DC
        self.E_Uband = E_eig
        self.E_int = E_int
        self.E_DC = E_DC
...
class Lichtenstein2_term(Hubbard_matrix_term):
    ...
    def _V_and_E_site(self, U, J, rho):
        ...
        Vd = np.zeros(10)
        Vd[::2] += J * (np.dot(A_mat, rd_up) +
                        np.dot(B_mat, rd_dn)) - 0.5 * J * (n_up - n_dn)
        Vd[1::2] += J * (np.dot(A_mat, rd_dn) +
                         np.dot(B_mat, rd_up)) + 0.5 * J * (n_up - n_dn)
    ...
    def V_and_E(self, rho):
        ...
        for g, UJ_ind in self._corr_group.items():
            ...
            ind = np.ix_(ids, ids)
            rho_site = rho.block(ids)
            Vee_site, E_site = self._V_and_E_site(U, J, rho_site)
            Vee[ind] += Vee_site
            E_int += E_site
