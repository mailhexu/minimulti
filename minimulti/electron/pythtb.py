...
class tb_model(object):
    ...
    def __init__(self,dim_k,dim_r,lat=None,orb=None,per=None,nspin=1):
        ...
        if lat is None:
            ...
            warnings.warn(" Lattice vectors not specified! I will use identity matrix.")
        ...
        if orb is None:
            ...
            warnings.warn(" Orbital positions not specified. I will assume a single orbital at the origin.")
        ...
        self._nsta=self._norb*self._n1
        ...
        #self._hoppings=[]
        self._hoppings=defaultdict(dict) # "{R: {(i, j):val}}"
    ...
    def set_onsite(self,onsite_en,ind_i=None,mode="set"):
        ...
        :param onsite_en: Either a list of on-site energies (in
          ...
          the case when *n1* is *1* (spinless) then each on-site
          energy is a single number.  If *n1* is *2* then on-site
    ...
    def set_hop(self,hop_amp,ind_i,ind_j,ind_R=None,mode="set",allow_conjugate_pair=False):
        i=int(ind_i)
        j=int(ind_j)
        R=(int(ind_R[0]), int(ind_R[1]), int(ind_R[2]))
        self._hoppings[R][(i, j)]=hop_amp

    def set_hop_orig(self,hop_amp,ind_i,ind_j,ind_R=None,mode="set",allow_conjugate_pair=False):
        r"""
        ...
        :param hop_amp: Hopping amplitude; can be real or complex
          number, equals :math:`H_{ij}({\bf R})`. If *n1* is *2*
...
""")
    ...
    def _val_to_block(self,val):
        """If n1=2 then returns a 2 by 2 matrix from the input
        ...
        matrix, just return it.  If n1=1 then just returns val."""
        # spinless case
        if self._n1==1:
            return val
        # spinfull case
...
matrix.""")            
    ...
    def display(self):
        ...
        print('number of spin components   =',self._n1)
    ...
    def _gen_ham_sparse(self, k_input=None):
        kpnt=np.array(k_input)
        #ham=np.zeros((self._norb,self._norb),dtype=complex)
        ham=sp.lil_matrix(shape=(self._norb, self_norb),dtype=complex)
        for i in range(self._norb):
            ham[i,i]=self._site_energies[i]
        for R, ijv in self._hoppings.items():
            expR=np.exp(2.0j*np.pi*np.dot(R, kpnt))
            #phase=np.exp((2.0j)*np.pi*np.dot(kpnt,rv))
            #print(expR)
            for key, amp in ijv.items():
                i, j= key
                amp=amp*expR
                ham[i,j]+=amp
                ham[j,i]+=amp.conjugate()
        ham=sp.csr_matrix(ham)
        return ham


    def _gen_ham(self, k_input=None, sparse=True, convention=2):
        kpnt=np.array(k_input)
        ham=np.zeros((self._norb,self._norb),dtype=complex)
        for i in range(self._norb):
            ham[i,i]=self._site_energies[i]
        phase=np.ones((self._norb,self._norb), dtype=complex)
        for R, ijv in self._hoppings.items():
            expR=np.exp(2.0j*np.pi*np.dot(R, kpnt))
            if isinstance(ijv, dict):
                for key, amp in ijv.items():
                    i, j= key
                    amp=amp*expR
                    ham[i,j]+=amp
                    ham[j,i]+=amp.conjugate()
            else: # ijv is a matrix
                ijvp=ijv*expR
                ham+=ijvp+ijvp.T.conjugate()
        if convention==1:
            n=len(self._orb)
            for i in range(n):
                for j in range(n):
                    phase[i,j]=np.exp(2.0j*np.pi*np.dot(self._orb[j]-self._orb[i], kpnt))
            ham=ham*phase
        return ham

    def _gen_ham_orig(self,k_input=None):
    ...
    def solve_all(self,k_list=None,eig_vectors=False, convention=2):
        r"""
        ...
        :returns:
          ...
          * **evec** -- Three dimensional array of eigenvectors for
            all bands and all kpoints. If *n1* equals 1 the format
            ...
            :download:`notes on tight-binding formalism
            <misc/pythtb-formalism.pdf>`. # n: band, j: orbital

            In the case when reciprocal space is zero-dimensional (as in a
            molecule) kpoint index is dropped and *evec* is of the format
            evec[band,orbital].

            In the spinfull calculation (*n1* equals 2) evec has
        ...
        if not (k_list is None):
            ...
            for i,k in enumerate(k_list):
                ...
                if eig_vectors==False:
                    eval=self._sol_ham(ham,eig_vectors=eig_vectors, convention=convention)
                    ret_eval[:,i]=eval[:]
                else:
                    (eval,evec)=self._sol_ham(ham,eig_vectors=eig_vectors, convention=convention)
                    ret_eval[:,i]=eval[:]
                    if self._n1==1:
                        ret_evec[:,i,:]=evec[:,:]
                    elif self._n1==2:
    ...
    def make_supercell(self, sc_red_lat, return_sc_vectors=False, to_home=True):
        ...
        sc_tb=tb_model(self._dim_k,self._dim_r,sc_cart_lat,sc_orb,per=self._per,nspin=self._n1)
    ...
    def position_matrix(self, evec, dir):
        ...
        :param evec: Eigenvectors for which we are computing matrix
          ...
          is evec[band,orbital] if *n1* equals 1 and
          evec[band,orbital,spin] if *n1* equals 2.
    ...
    def position_hwf(self,evec,dir,hwf_evec=False,basis="orbital"):
        ...
        :param basis: Optional parameter. If basis="bloch" then hybrid
          ...
          in the i-th hybrid Wannier function.  If basis="orbital" and n1=1 then
          hwf[i,orb] correspond to the weight of orb-th orbital in the i-th 
          hybrid Wannier function.  If basis="orbital" and n1=2 then
        ...
        :returns:
          ...
          * **hwf** -- Eigenvectors of the position operator matrix :math:`X`.
            ...
            and *n1*.  If *basis* is "bloch" then x refers to indices of 
            Bloch states *evec*.  If *basis* is "orbital" then *x* (or *x* and *s*)
            correspond to orbital index (or orbital and spin index if *n1* is 2).
...
class wf_array(object):
    ...
    *Parametric or irregular k-space grid grid*:
    ...
    def __init__(self,model,mesh_arr):
        ...
        if self._n1==2:
            wfs_dim=np.append(wfs_dim,self._n1)            
    ...
    def solve_on_grid(self,start_k):
        ...
        if self._norb<=1:
            ...
            gap_dim=np.append(gap_dim,self._norb*self._n1-1)
                ...
                for P in self.ham_r:
                    if not (R[0]==0 and R[1]==0 and R[2]==0):
                        ...
                        if R[0]==-P[0] and R[1]==-P[1] and R[2]==-P[2]: 
                            ...
                            found_pair=True
                if found_pair==False:
                    raise Exception("Did not find negative R for R = "+str(R)+"!")
