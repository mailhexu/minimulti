...
class Occupations(object):
    ...
    def root_function(self, mu):
        ...
        f = self.fermi(mu)
        return np.einsum('i, ijk->', self.wk, f) - self.nel
    ...
    def occupy(self, e, xtol=1e-11):
        ...
        try:
            ...
            mu = brentq(
                self.root_function, guess - dmu, guess + dmu, xtol=xtol)
            #mu = brent(
            #    self.root_function,
        ...
        if np.abs(self.root_function(mu)) > xtol * 1e4:
            ...
            #)
            print('Fermi level could not be assigned reliably. Has the system fragmented?')
...
class GaussOccupations(Occupations):
    ...
    def occupy(self, e, xtol=1e-8, guess=0.0):
        self.e = e
        dos = myDOS(
            kweights=self.wk, eigenvalues=e, width=self.width, npts=501)
