...
from minimulti.electron.density import gen_density_matrix
...
class mytbmodel(tb_model):
    ...
    def __init__(self, dim_k, dim_r, lat=None, orb=None, per=None, nspin=1):
        super(mytbmodel, self).__init__(dim_k=dim_k,
                                        dim_r=dim_r,
                                        lat=np.array(lat),
                                        orb=orb,
                                        per=per,
...
class etb_model(mytbmodel):
    def __init__(self,
        ...
        self._nso = len(orb)
        self._nspin = nspin
        self._norbital = self._nso // self._nspin
        self._nband = self._norbital
        self._actual_nspin = nspin

        # density matrix
        self._rho = gen_density_matrix(self._nso, diag_only=False)
        self._has_rho = False
        self._has_ham_U = False
        # Hamiltonian in k space. [nkpts, norbital, norbital]
        self._ham0_k = None
        # effective potential of Hubbard Term. Note that H_U is not k-dependent.
        self._ham_U = np.zeros((self._nso, self._nso), dtype=float)
        self._U_energy = 0.0
        self._Uband_energy = 0.0
        self._int_energy = 0.0
        self._DC_energy = 0.0
        self._spinat = np.zeros(self._norbital)
        self._nel = 0
    ...
    def save_density(self, fname=None, rhok_fname=None):
        ...
        self._rho.save(fname)

    def load_density(self, fname=None):
        if fname is None:
            fname = self._density_file
        self._rho.load(fname)
        self._has_rho = True
    ...
    def get_eigenvalues(self, kpt=0, spin=None, refresh=False):
        ...
        if spin is None or self._actual_nspin == 1:
            return self._eigenvals[kpt, 0, :]
        else:
            return self._eigenvals[kpt, spin, :]

    def get_bandgap(self):
        flat_eig = np.squeeze(self._eigenvals, axis=0)
        eig = np.sort(flat_eig, axis=1)
        CBM = np.min(eig[self._nel, :])
        VBM = np.max(eig[self._nel - 1, :])
        Eg = CBM - VBM - self._width
        if Eg < 0:
            Eg = 0
        return Eg
    ...
    def get_pdos(self, emin, emax, nedos, sigma=None, fname=None):
        ...
        self.pdos = PDOS(kpts=self._kpts,
    ...
    def set_constraint(self, V, charge_dict):
        term = SiteChargeConstraint(bset=self.bset,
                                    V=V,
    ...
    def _gen_ham0_k(self, convention=2):
        ...
        """
        self._ham0_k = np.zeros(shape=(len(self._kpts), self._nso, self._nso),
                                dtype=complex)
        for ik, k in enumerate(self._kpts):
            self._ham0_k[ik, :, :] = self._gen_ham(k_input=k,
    ...
    def _sol_ham(self, ham, eig_vectors=False):
        ...
        if self._nspin == 1:
            ...
            ham_use = ham.reshape((2 * self._norbital, 2 * self._norbital))
        ...
        if eig_vectors == False:  # only find eigenvalues
            ...
            evals[1] = np.linalg.eigvalsh(ham_use[1::2, 1::2])
            ret = evals
        else:  # find eigenvalues and eigenvectors
            evals = np.zeros((self._nspin, self._nband), dtype=float)
            evecs = np.zeros((self._nspin, self._norbital, self._nband),
                             dtype=complex)
            if self._nspin == 2:
                evals[0], evecs[0] = eigh(ham_use[::2, ::2])
                evals[1], evecs[1] = eigh(ham_use[1::2, 1::2])
            ret = (evals, evecs)
        return ret
    ...
    def solve_all(self,
        ...
        ret_eval = np.zeros((nkp, self._nspin, self._nband), dtype=float)
        ret_evec = np.zeros((nkp, self._nspin, self._norbital, self._nband),
                            dtype=complex)

        for ik, kpt in enumerate(k_list):
            # setup_hamiltonian
            if has_ham0:
                ham0 = self._ham0_k[ik, :, :]
            else:
            ...
            if not eig_vectors:
                ret_eval[ik] = self._sol_ham(h_total_k,
                                             eig_vectors=eig_vectors)
            else:
                ret_eval[ik], ret_evec[ik] = self._sol_ham(
    ...
    def get_ham0_energy(self):
        ...
        for ik, kpt in enumerate(self._kpts):
            evecs = self._eigenvecs[ik, :, :]  # Note ib, ik , iorb.
    ...
    def get_rho(self):
        return self._rho.rho

    def get_charges(self):
        return self._rho.density

    def get_magnetic_moments(self):
        return self._rho.spin_for_each_orbital()
    ...
    def scf_solve(
        ...
        if self._U_term is None:
            ...
            self.scf_solve_U(max_step=max_step,
    ...
    def scf_solve_U(
        ...
        if self._load_density and os.path.exists(self._density_file):
            ...
            self.solve_all(eig_vectors=True,
                           convention=convention,
                           zeeman=False)
            self.get_occupations(self._nel, self._width, refresh=False)
            self.get_density_matrix()
        ...
        for i in range(max_step):
            ...
            last_rho = copy.deepcopy(self._rho.rho)
            self.get_density_matrix()
            ...
            delta_rho = np.abs(
                np.asarray(last_rho) - np.asarray(self._rho.rho)).max()

            last_rho_in = np.array(last_rho)
            rho_in = np.array(self._rho.rho)
            shape = rho_in.shape
            conv_rho, rho_out = mixer(last_rho_in.flatten(), rho_in.flatten())
            self._rho.rho = rho_out.reshape(shape)
    ...
    def save_result(self, pfname=None):
        ...
        #self._results['bandgap'] = self.get_bandgap()
        if self._has_ham_U:
    ...
    def unfold(self, kpts, sc_matrix, evals=None, evecs=None):
        if evals is None and evecs is None:
            evals, evecs = self.solve_all(k_list=kpts,
                                          eig_vectors=True,
                                          convention=1)
        positions = self._orb
    ...
    def plot_band(self,
        ...
        if supercell_matrix is not None:
            kvectors = [np.dot(k, supercell_matrix) for k in kvectors]
        band = bandpath(kvectors, self.get_cell(), npoints)
        kpts = band.kpts
        x, X, labels = band.get_linear_kpoint_axis()
        evalues, evecs = self.solve_all(k_list=kpts, eig_vectors=True)
        if shift_fermi:
            evalues -= self.get_fermi_level()
        if split_spin and self._actual_nspin == 2:
            evalues_up, evalues_dn = evalues[:, 0, :], evalues[:, 1, :]
    ...
    def get_projection(self, orb, spin=0, eigenvecs=None):
        ...
        if eigenvecs is None:
            ...
            return eigenvecs[:, spin, orb, :] * eigenvecs[:, spin,
                                                          orb, :].conjugate()
    ...
    def plot_COHP_fatband(self,
        ...
        if efermi is None:
            efermi = self.get_fermi_level()
        ax = band_cohp.plot_COHP_fatband(kpts=kpts,
                                         k_x=x,
                                         X=X,
                                         width=width,
                                         iblock=iblock,
    ...
    def get_epc_dos(self,
                    ...
                    sigma=0.1):
        evals = self._eigenvals
        weights = epc.get_band_shift(self._kpts,
                                     self._eigenvecs,
                                     evals=evals,
                                     onsite=onsite,
                                     order=order)
    ...
    def plot_epc_dos(
        ...
        e, dos, idos = self.get_epc_dos(epc,
                                        onsite=onsite,
                                        order=order,
                                        emin=emin,
                                        emax=emax,
...
class atoms_model(etb_model):
    def __init__(self, atoms=None, basis_dict=None, basis_set=None, nspin=2):
        if atoms is not None:
            ...
            if basis_dict is not None:
                self.bset = atoms_to_basis(atoms,
    ...
    def set_Hubbard_U(self,
        ...
        if Utype == 'SUN' or Utype == 0:
            ...
            uterm = Dudarev_term(bset=self.bset,
                                 Hubbard_dict=Hubbard_dict,
                                 DC=DC,
                                 DC_shift=DC_shift,
                                 DC_type=DC_type)
            ...
            #    DC_shift=DC_shift,
            #    DC_type=DC_type)
            uterm = Lichtenstein_term(bset=self.bset,
                                      Hubbard_dict=Hubbard_dict,
                                      DC=DC,
                                      DC_shift=DC_shift,
                                      DC_type=DC_type)
            ...
            raise ValueError(
                ...
                % Utype)
        self._rho = gen_density_matrix(self._nso, self._diag_only)
    ...
    def make_supercell(self, sc_matrix, ijR=False):
        ...
        sc_model = atoms_model(atoms=sc_atoms,
                               basis_set=sc_bset,
                               nspin=self._actual_nspin)
        ...
        if ijR:
            sc_hoppings = defaultdict(lambda: np.zeros(
                (sc_model._nso, sc_model._nso), dtype=float))

            sc_model._hoppings = smaker.sc_RHdict(self.data, self.nbasis)
        else:
            hop_dict = dict()
        ...
        if self.Utype is not None:
            sc_model.set_Hubbard_U(Utype=self.Utype,
                                   Hubbard_dict=self.Hubbard_dict)
        return sc_model
    ...
    def get_site_nelectron(self, spin=None):
        ...
        for b in self.bset:
            n[b.site, b.spin] += self._rho.diag_i(b.index)
        if spin is None:
    ...
    def plot_piecewise_linear(self, U, J, step=0.25, axis=None):
        ...
        n = len(nels) - 1
        mid = round(n / 2)
        linfunc = interp1d(x=[nels[0], nels[mid], nels[-1]],
...
def test_atoms_model():
    ...
    model.set_Hubbard_U(Utype='Kanamori', Hubbard_dict={'H': {'U': 9, 'J': 0}})
    ...
    scmodel.scf_solve()
    print(f"rho: {scmodel._rho.rho}")
    print(f"charges: {scmodel.get_charges()}")
    print(f"spins: {scmodel.get_magnetic_moments()}")
