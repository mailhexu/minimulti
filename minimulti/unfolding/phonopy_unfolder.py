...
def unf(phonon, sc_mat, qpoints, knames=None, x=None, xpts=None):
    ...
    prim = Atoms(symbols=prim.get_chemical_symbols(),
                 cell=prim.get_cell(), positions=prim.get_positions())
    # vesta_view(prim)
    ...
    uf = phonon_unfolder(atoms=prim, supercell_matrix=sc_mat,
    ...
    ax = plot_band_weight([list(x)]*freqs.shape[1], freqs.T*33.356,
                          weights[:, :].T*0.99+0.001, xticks=[knames, xpts], style='alpha')
    return ax
...
def phonopy_unfold(sc_mat=np.diag([1, 1, 1]), unfold_sc_mat=np.diag([3, 3, 3]), force_constants='FORCE_CONSTANTS', sposcar='SPOSCAR', qpts=None, qnames=None, xqpts=None, Xqpts=None):
    phonon = read_phonopy(
        sc_mat=sc_mat, force_constants=force_constants, sposcar=sposcar)
    ax = unf(phonon, sc_mat=unfold_sc_mat, qpoints=qpts,
             knames=qnames, x=xqpts, xpts=Xqpts)
    return ax
...
def test():
    phonon = read_phonopy(sc_mat=np.diag(
        [1, 1, 1]), force_constants='FORCE_CONSTANTS', sposcar='SPOSCAR')
    qpts, x, X, names = kpath()
    ax = unf(phonon, sc_mat=np.diag([3, 3, 3]),
             qpoints=qpts, knames=names, x=x, xpts=X)
    plt.show()
